...
//
//		WindowsButton removeBtn = new WindowsButton("Remove");
//		removeBtn.setWidth(100);
//		removeBtn.setHeight(100);
//		removeBtn.setPosition(true, 400, 150);
//		removeBtn.onButtonClicked.addCallback(this::onRemoveButtonClicked);
//		stage.hudLayer.addChild(removeBtn);
	}

	private void onRemoveButtonClicked(InteractableContainer btn)
	{
		int len = nodeList.size();
		int index = (int) (Math.random() * len);

		List<WindowsTreeNode<Integer>> list = new ArrayList<>();
		
		WindowsTreeNode<Integer> node = nodeList.get(index);
		node.getChildren(list, true);
