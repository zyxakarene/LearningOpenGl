...
public class BaseHud extends JsonSprite
{
	...
	protected void onComponentsCreated()
	{
		...
		debugPos.setTexture(new TextureFromInt(SIZE, SIZE, renderer.positionInt(), TextureSlot.SHARED_DIFFUSE));
		addChild(debugPos);

		Image debugNorm = new Image();
		debugNorm.setScale(1, -1);
		debugNorm.setPosition(true, SIZE * 1, GameConstants.GAME_HEIGHT);
		debugNorm.setTexture(new TextureFromInt(SIZE, SIZE, renderer.normalInt(), TextureSlot.SHARED_DIFFUSE));
		addChild(debugNorm);
		...
		debugScreenPos.setTexture(new TextureFromInt(SIZE, SIZE, renderer.screenPositionInt(), TextureSlot.SHARED_DIFFUSE));
		...
		debugScreenNorm.setTexture(new TextureFromInt(SIZE, SIZE, renderer.screenNormalInt(), TextureSlot.SHARED_DIFFUSE));
		...
		debugCol.setTexture(new TextureFromInt(SIZE, SIZE, renderer.colorInt(), TextureSlot.SHARED_DIFFUSE));
		addChild(debugCol);

		Image debugDepth = new Image();
		debugDepth.setScale(1, -1);
		debugDepth.setPosition(true, SIZE * 3, GameConstants.GAME_HEIGHT);
		debugDepth.setTexture(new TextureFromInt(SIZE, SIZE, renderer.depthInt(), TextureSlot.SHARED_DIFFUSE));
		addChild(debugDepth);

		Image debugShadow = new Image();
		debugShadow.setScale(1, -1);
		debugShadow.setPosition(true, SIZE * 4, GameConstants.GAME_HEIGHT);
		debugShadow.setTexture(new TextureFromInt(SIZE, SIZE, depthRenderer.depthInt(), TextureSlot.SHARED_DIFFUSE));
